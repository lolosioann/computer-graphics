...
def test_triangle_on_edge():
    # Mock image (white canvas)
    img = np.ones((500, 500, 3), dtype=np.float32)
    vertices = np.array([[100, 200], [400, 0], [100, 100]])
...
def test_infinite_slope():
    ...
    test_img = f_shading(
        img,
        vertices=[[100, 100], [400, 200], [400, 300]],
        vcolors=[[0, 0, 1], [0, 1, 0], [1, 0, 0]],
    )
    # Scale the image to [0, 255] and convert to uint8
    test_img = (test_img * 255).clip(0, 255).astype(np.uint8)
    ...
    test_img = cv2.cvtColor(test_img, cv2.COLOR_RGB2BGR)
    # Show image
    cv2.imshow("infinite slope", test_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    assert 1 == 1